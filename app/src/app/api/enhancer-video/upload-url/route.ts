/**
 * POST /api/enhancer-video/upload-url
 *
...
import { VIDEO_MAX_SIZE_BYTES, VIDEO_UPLOAD_PRESIGNED_EXPIRY_SECS } from '@/constants/storage';
import { getPresignedPutUrl, inputKey } from '@/helpers/enhancer-video/storage';
import { db } from '@/libs/core/DB';
import { videos } from '@/models/Schema';
...
export async function POST(req: NextRequest) {
  ...
  if (sizeBytes && sizeBytes > VIDEO_MAX_SIZE_BYTES) {
  ...
  const [uploadUrl, expiresAt] = await Promise.all([
    getPresignedPutUrl(key, mimeType, VIDEO_UPLOAD_PRESIGNED_EXPIRY_SECS),
    new Date(Date.now() + VIDEO_UPLOAD_PRESIGNED_EXPIRY_SECS * 1000),
  ]);
