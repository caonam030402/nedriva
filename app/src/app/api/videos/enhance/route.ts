 ...
 * 2. Reads the video record (already uploaded by /api/videos/upload)
...
import type { VideoEnhancementOptions } from '@/models/VideoEnhancementSchema';
...
import {
  ...
  userVideoUsage,
  videos,
} from '@/models/VideoEnhancementSchema';
...
export async function POST(req: NextRequest) {
  ...
  const VALID_STYLE = Object.values(EEnhancementStyle);
  ...
  try {
    await enqueueVideoProcessing(jobId, video.inputUrl, destKey, {
      upscale_factor: opts.upscaleFactor as 'auto' | '2x' | '4x',
    ...
    const { refundUserCredits } = await import('@/libs/persistence/users/refundUserCredits');
