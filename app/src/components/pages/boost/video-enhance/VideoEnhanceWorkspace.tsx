...
function ProcessingProgress(props: {
  progress: number;
...
export function VideoEnhanceWorkspace() {
  ...
  const { data: statusData } = useVideoJobStatus(
    uploadedVideoId,
    jobId,
    ...
    <div className="flex h-full min-h-0 w-full flex-1 flex-col overflow-hidden lg:flex-row">
              ...
              <div className="relative min-h-0 flex-1 overflow-hidden rounded-xl border border-white/10 bg-black/40">
                ...
                {isFailed && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-black/70 p-4 text-center backdrop-blur-sm">
                    ...
                    <p className="text-sm font-medium text-danger">{errorMsg ?? 'Processing failed'}</p>
      ...
      <aside className="flex w-full shrink-0 flex-col border-white/8 bg-page lg:w-[min(100%,400px)] lg:border-l">
        <div className="max-h-[55vh] min-h-0 flex-1 overflow-y-auto overscroll-y-contain lg:max-h-none">
          ...
          <div className="border-b border-white/8 px-4 py-4">
            ...
            <div className="flex gap-px overflow-hidden rounded-lg border border-white/10 bg-surface p-0.5">
              <FormatPill id="auto"  label={t('format_auto')}  tooltip={t('format_auto_tip')}  selected={format === 'auto'}  onSelect={() => setFormat('auto')} />
              <FormatPill id="mp4"   label={t('format_mp4')}   tooltip={t('format_mp4_tip')}   selected={format === 'mp4'}   onSelect={() => setFormat('mp4')} />
              <FormatPill id="webm"  label={t('format_webm')}  tooltip={t('format_webm_tip')}  selected={format === 'webm'}  onSelect={() => setFormat('webm')} />
              <FormatPill id="mov"   label={t('format_mov')}   tooltip={t('format_mov_tip')}   selected={format === 'mov'}   onSelect={() => setFormat('mov')} />
            </div>
          </div>
        ...
        <div className="border-t border-white/8 p-4">
          ...
          {!file && (
            <p className="mt-2 text-center text-[11px] text-muted">{t('cta_note')}</p>
          )}
