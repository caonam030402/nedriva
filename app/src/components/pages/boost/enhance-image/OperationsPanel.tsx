...
import type { EMoreModel, ESizeMode } from '@/enums/enhancer';
...
import { EBgType } from '@/enums/enhancer';
import { Link } from '@/libs/i18n/I18nNavigation';
import { buildRenderConfig, SCALE_FACTORS, UPSCALE_MODELS } from './constants';
      ...
      <div className="isolate min-h-0 touch-pan-y overflow-y-auto overscroll-y-contain [-webkit-overflow-scrolling:touch]">
        ...
        >
          {ops.upscaleEnabled && (
            <>
              <PillGroup
                options={UPSCALE_MODELS.map(m => ({ id: m.id, label: m.label, tooltip: m.tooltip, previewUrl: m.previewUrl }))}
                  ...
                  lockedTooltip: m.locked
                    ? (
                        <p className="text-xs leading-relaxed text-gray-800">
                          {t('paid_tooltip_text')}
                          {' '}
                          <Link href="/pricing" className="font-medium text-brand-light hover:underline">{t('see_pricing')}</Link>
                        </p>
                      )
                    : undefined,
                }))}
            ...
            {render.showFactor && (
              <div className="border-b border-white/8 px-4 py-4">
                ...
                <PillGroup
                  ...
                  onChange={id => setOps(s => ({ ...s, scaleFactor: id as typeof ops.scaleFactor }))}
                />
              </div>
            )}
