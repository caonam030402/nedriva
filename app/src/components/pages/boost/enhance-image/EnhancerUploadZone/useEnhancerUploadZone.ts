...
import { EQueueStatus, ESizeMode } from '@/enums/enhancer';
...
import { countEnhancerReadyPlusProcessingRows } from '../countEnhancerProcessingRows';
import {
  enhancerRunToViewModel,
  queueItemToViewModel,
} from '../queueRowViewModel';

export type EnhancerTableRow = { kind: 'local'; item: QueueItem } | { kind: 'server'; run: EnhancerRunItem };

export type EnhancerPreviewTarget = { kind: 'local'; id: string } | { kind: 'server'; runId: string };
...
export function useEnhancerUploadZone() {
  ...
  const scaleFactor = ops.upscaleEnabled
    ? (ops.sizeMode === ESizeMode.Scale ? Number(ops.scaleFactor) : 4)
    : 1;
  ...
  const tableRows: EnhancerTableRow[] = useMemo(() => {
    ...
    const serverOnly = runsItems.filter(
      r =>
        r.deletedAt == null
        && (r.clientQueueItemId == null || !localIds.has(r.clientQueueItemId)),
    );
  ...
  const addFiles = (files: File[]) => {
    ...
    const pipelineSlots = Math.max(
      0,
      ENHANCER_MAX_CONCURRENT_PROCESSING - activePipeline,
    );
