...
import { EBgType, EMoreModel, EScaleFactor, ESizeMode, EUpscaleModel } from '@/enums/enhancer';
...
export const UPSCALE_MODELS: { id: EUpscaleModel; label: string; tooltip: string; previewUrl: string }[] = [
  {
    id: EUpscaleModel.Prime,
    label: 'Prime',
    tooltip: 'Our most realistic upscaler. Best for portraits, skin, and fine textures. Excels at product, fashion, and food photography.',
    previewUrl: 'https://images.unsplash.com/photo-1531746020798-e6953c6e8e04?w=480&q=80',
  },
  {
    id: EUpscaleModel.Gentle,
    label: 'Gentle',
    tooltip: 'Subtle enhancement that preserves the original. Good for product photos and images with small text (former "Image with Text").',
...
export const MODEL_CONFIGS: Record<EUpscaleModel, ModelConfig> = {
  ...
  [EUpscaleModel.OldPhoto]: { sizeModes: [ESizeMode.Auto, ESizeMode.Custom], disableLightAI: true, disableRemoveBg: true },
  [EUpscaleModel.TryAll]: { sizeModes: [ESizeMode.Auto, ESizeMode.Custom], disableLightAI: true, disableRemoveBg: true },
  [EUpscaleModel.More]: { sizeModes: [ESizeMode.Auto, ESizeMode.Scale, ESizeMode.Custom], subModels: MORE_MODELS },
};
 ...
 */
export function buildRenderConfig(model: EUpscaleModel, currentSizeMode: ESizeMode, isPaidUser: boolean) {
