import type { EnhancerJobStatusBody } from '@/types/enhancer-image/jobApi';
import { apiRoutes } from '@/constants/apiRoutes';

/**
 * GET `/api/enhancer-image/jobs/[jobId]` — poll job status.
...
export async function fetchEnhancerJobStatus(
  ...
  const res = await fetch(apiRoutes.enhancerImage.job(jobId), { credentials: 'include' });
  if (!res.ok) {
    return null;
  }
