/**
 * Typed HTTP client for the Python enhancer-image processing service.
 ...
 * @param queueItemId - Client-provided correlation ID
 * @param ops - Enhancement options state
 */
export async function enqueueProcessing(
  ...
  const res = await fetch(`${BASE_URL}/api/v1/image-enhancer/process`, {
    method: 'POST',
    headers: headers(),
...
export async function getJobStatus(jobId: string): Promise<JobStatusResponse> {
  const res = await fetch(`${BASE_URL}/api/v1/image-enhancer/jobs/${jobId}`, {
    headers: headers(),
