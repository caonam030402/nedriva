/**
 * DB persistence for Enhancer jobs (`enhancer_processed_images`).
 */
 ...
 * @param body
 */
export async function updateEnhancerJobFromWebhook(
  body: WebhookProcessBody,
): Promise<void> {
...
export async function listDoneEnhancerJobsForUser(
  ...
  return db
    ...
    .orderBy(
      desc(enhancerProcessedImages.createdAt),
      desc(enhancerProcessedImages.id),
    )
    .limit(take);
}
...
export async function listEnhancerRunsForUserPaged(
  ...
  const safeLimit = Math.min(Math.max(1, limit), MAX_RUNS_LIMIT);
  const safeRequestedPage = Math.max(1, page);
...
const MAX_ACTIVE_RUNS_LIMIT = 50;

/**
 * Queued / processing rows only — keep polling/resume working when the main runs list is offset-paged.
 */
export async function listEnhancerRunsActiveForUser(userId: string, limit: number = 30) {
  const safeLimit = Math.min(Math.max(1, limit), MAX_ACTIVE_RUNS_LIMIT);
